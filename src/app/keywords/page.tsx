      ...
      <div className="page-head">
        <div>
          ...
          <p className="lede">
            ...
            can honour a date cutoff; keywords go through ranked search and are
            best-effort. Yield columns fill in as runs complete, so terms that cost
            requests without returning listings become visible and prunable.
          </p>
        </div>
      </div>

      <form action={addKeywords} className="toolbar" style={{ marginBottom: "var(--s5)" }}>
        <div className="field" style={{ flex: "1 1 380px" }}>
          ...
          <textarea
            ...
            name="terms"
            rows={2}
            placeholder="offmarket, presale, comingsoon"
          />
        </div>
      ...
      {all.length === 0 ? (
        <div className="empty">
          ...
          <p className="small" style={{ margin: 0 }}>
            Nothing will be collected until at least one term is added and enabled.
          </p>
        </div>
      ) : (
          ...
          <table>
            ...
            <tbody>
              {all.map((keyword) => {
                ...
                return (
                  <tr key={keyword.id} style={{ opacity: keyword.enabled ? 1 : 0.55 }}>
                    <td>
                      <b>{keyword.kind === "hashtag" ? `#${keyword.term}` : keyword.term}</b>
                    </td>
                    <td className="small faint">{keyword.kind}</td>
                    <td>
                    ...
                    <td className="num">
                      {wins > 0 ? (
                        <span className="tag ok">{wins}</span>
                      ) : (
